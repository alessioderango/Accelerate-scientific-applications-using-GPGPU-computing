...
template <class T>
void drawWithAllegro(T* p,int nRows,int nCols, int step){
        double maxValue = p[0];
        for(int row=0;row<nRows;row++){
            for(int col=0;col<nCols;col++){                  
                
                double value= p[row*nCols+col];
                if(value > maxValue)
                {
                  maxValue = value;
                }
                if (value > 0 )
                {
                  ...
                  rectfill(buffer,
                           col*pixelsQuadrato, 
                           row*pixelsQuadrato, 
                           col*pixelsQuadrato+pixelsQuadrato,
                           row*pixelsQuadrato+pixelsQuadrato, 
                           makecol((value*255)/30, 100, 100));
                }else
                {
                          rectfill(buffer,
                           col*pixelsQuadrato, 
        ...
        textprintf_ex(buffer,font , 150,0, makecol(255,255,255), makecol(0,0,0), "step %d, max value %f ", step, maxValue);
...
int main(int argc, char **argv)
{
  ...
  setAllegroDimension(cols*pixelsQuadrato, rows*pixelsQuadrato);
